-- バランスの取れた撮影会テストデータ
-- 現在日付を基準として過去・現在・未来の撮影会を作成

-- 既存のテストデータを削除（必要に応じて）
-- DELETE FROM photo_sessions WHERE title LIKE '%テスト%' OR title LIKE '%Test%';
...
DECLARE
    ...
    session_id uuid;
BEGIN
    -- 管理者ユーザーIDを取得（存在しない場合は作成）
    SELECT id INTO organizer_id FROM profiles WHERE email = 'admin@shutterhub.app' LIMIT 1;
    
    IF organizer_id IS NULL THEN
        -- 管理者プロフィールを作成
        INSERT INTO profiles (id, email, display_name, user_type, role)
        VALUES (gen_random_uuid(), 'admin@shutterhub.app', 'ShutterHub運営', 'organizer', 'admin')
